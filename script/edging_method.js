function edging_method() {
	...
	for(var i = 2; i<=n; i++) {
		beta[i] = D[i-1].mult(u[i]);
		gamma[i] = v[i].mult(D[i-1]);
		alpha[i] = 0;
		for(var k = 1;k<n;k++) {
		...
		for(var j = 1; j<i; j++) {
			...
			D[i].set(i,j,gamma[i].get(j)/alpha[i]);
			for(var k = 1; k<i; k++) {
				D[i].set(j,k,D[i-1].get(j,k)+beta[i].get(j)*gamma[i].get(k)/alpha[i]);
			}
		}
